...
def run(
    ...
    # Initialize crosswalk tools and find crosswalks
    crosswalk_tools = crosswalk_detection.CrossWalkStopDetection(source)
    crosswalk_tools.detect_crosswalks()
            ...
            if det is not None and len(det):
                ...
                if len(outputs[i]) > 0:
                    for j, (output) in enumerate(outputs[i]):
                        ...
                        if names[int(cls)] in ['truck', 'car', 'bus', 'motorbike']:

                            bbox_w = output[2] - output[0]
                            bbox_h = output[3] - output[1]
                            size = bbox_w * bbox_h
                            size_factor = np.sqrt(size)

                            center = crosswalk_tools.bounding_box_center(bboxes)
                            crosswalk_tools.handle_detection(names[int(cls)]+str(id), center, size_factor)
